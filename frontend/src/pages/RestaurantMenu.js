...
const CATEGORIES = [
  'Appetizers','Main Course','Desserts','Beverages','Salads','Soups','Snacks','Breakfast','Sides'
];
...
export default function RestaurantMenu() {
  ...
  const restaurantId = useMemo(() => restaurant?.id || restaurant?._id, [restaurant]);
  ...
  const [formData, setFormData] = useState({
    name: '', description: '', price: '', category: 'Main Course', image: '', available: true
  });
  const [filterCategory, setFilterCategory] = useState('All');

  useEffect(() => {
    if (!isLoggedIn) { navigate('/restaurant-login', { replace: true }); return; }
  }, [isLoggedIn, navigate]);

  useEffect(() => {
    if (!restaurantId) return;
    (async () => {
      try { await fetchMenu(); } finally { setLoading(false); }
    })();
  }, [restaurantId]);

  const fetchMenu = async () => {
    const data = await restaurantAPI.getById(restaurantId); // expects { menu: [...] }
  ...
  const handleImageUpload = (e) => {
    ...
    if (!f) return;
    if (f.size > 5 * 1024 * 1024) { alert('Image size should be under 5MB'); return; }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        // ✅ pass (menuItemId, body)
        await restaurantAPI.updateMenuItem(editingItem._id, payload);
  ...
  const onDelete = async (itemId) => {
    ...
    try {
      await restaurantAPI.deleteMenuItem(itemId); // ✅ only id
      await fetchMenu();
  ...
  const onToggleAvailable = async (item) => {
    try {
      await restaurantAPI.updateMenuItem(item._id, { ...item, available: !item.available });
      await fetchMenu();
    } catch (err) {
  ...
  const handleClose = () => { setEditingItem(null); setShowModal(false); };

  const itemsToShow = (filterCategory === 'All')
    ? menuItems
  ...
  return (
    <div className="restaurant-menu-page">
      <div className="menu-container">
        ...
        <button className="add-item-btn" onClick={handleOpenAdd}>➕ Add Menu Item</button>
        </div>
        ...
        {itemsToShow.length === 0 ? (
          <div className="no-items"><h2>📋 No menu items yet</h2><p>Start building your menu by adding items</p></div>
        ) : (
                ...
                <div className="item-details">
                  ...
                  <div className="item-footer">
                    ...
                    <div className="item-actions">
                      <button className="toggle-btn" title={item.available ? 'Mark as unavailable' : 'Mark as available'} onClick={() => onToggleAvailable(item)}>
                        {item.available ? '✓' : '✗'}
                      </button>
